...
function convertSvgMap(svgMapProjectPath, generator) {
    ...
    parser.parseString(svg, function (err, result) {
        if (err === null) {
            fs.writeFileSync(svgJsonPath, generator(result))
        }
    });
}
convertSvgMap('/maps/GA.svg', generateGAJson);
convertSvgMap('/maps/US.svg', generateUSJSon);